...
// Funkcija za zamenu vrednosti svakog èvora sumom elemenata u njegovom levom i desnom podstablu
int replace(Node* root) {
    ...
    // Menjamo vrednost trenutnog èvora
    root->value = leftSum + rightSum;

    // Vraæamo staru vrednost èvora + sumu svih potomaka
...
int main() {
    Node* root = NULL;
    srand(time(0));  // Inicijalizacija generatora sluèajnih brojeva

    // Dodajemo sluèajne brojeve u stablo
    for (int i = 0; i < 10; i++) {
        int num = rand() % 81 + 10;  // Generiše brojeve u opsegu od 10 do 90
    ...
    // Menjamo vrednosti èvorova prema zahtevu
    replace(root);
