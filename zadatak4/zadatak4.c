...
int main()
{
	...
	// cita prvu liniju i pretvara ga u poly1
	fgets(buffer, MAX_LINE, filePointer);
	strToPoly(&poly1Head, buffer);

	// cita drugu liniju i pretvara je u poly2
	fgets(buffer, MAX_LINE, filePointer);
...
int addMonoToPoly(Mono* head, Mono* mono)
{
	...
	// Pronalazi prvi monom s eksponentom manjim ili jednakim eksponentu novog monoma
	...
	// Stavlja novi monom iza prethodnika
	else {
		mono->next = prev->next;
		prev->next = mono;
	}
